...
### 4. 上传图片报错 `user_access_token missing`
...
1. **#1 删除编辑按钮**（1 行代码，30 秒）
2. **#4 + #6 修 user_access_token**（create route + upload route 走 lark-cli user 身份）
3. **#2 列表首图**（查 TaskGrid.tsx 渲染逻辑）
4. **#3 zip 下载**（改 Blob 下载，加错误提示）
5. **#5 重连自动弹**（错误状态检测 + 弹模态）
6. **#8 列表排序**（4 个按钮 + sort 函数）
7. **#7 拖拽排序**（dnd-kit 接入）
8. **#11 顶部统计条**
